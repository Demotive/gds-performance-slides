...
if (offline === false) {
	var usersUrl = '/realtime/sorn'; // see server.rb
	var satisfactionUrl = '/satisfaction/sorn'; // see server.rb
} else {
	var usersUrl = 'assets/data/sorn-realtime-users.json';
	var satisfactionUrl = 'assets/data/sorn-customer-satisfaction.json';
}
...
var updateSatisfaction = function() {
	$.ajax({
		...
		success: function(d) {
			...
			var r = scoreToPercentage(d.data[d.data.length-1].satisfaction_sorn);
